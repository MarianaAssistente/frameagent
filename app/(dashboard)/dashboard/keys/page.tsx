...
import {
  ...
  RefreshCw, Shield, ExternalLink, Copy, Clock,
} from "lucide-react";
        ...
        <div className="flex items-center gap-3 mt-0.5">
          ...
          {apiKey.last_used_at && (
            <span className="text-[10px] text-white/20 flex items-center gap-1">
              <Clock size={9}/> Usado {new Date(apiKey.last_used_at).toLocaleDateString("pt-BR")}
            </span>
          )}
        </div>
      </div>
  ...
  return (
    <div className="p-5 rounded-2xl border border-[#C9A84C]/25 bg-[#C9A84C]/5 space-y-4">
      <div className="flex items-center justify-between">
        <p className="text-sm font-semibold text-white">Nova API Key</p>
      ...
      <form onSubmit={submit} className="space-y-3">
        ...
        <div>
          ...
          {cfg && (
            <p className="text-[10px] text-white/35 mt-2 flex items-center gap-1">
              {cfg.desc} ·{" "}
              <a href={cfg.docsUrl} target="_blank" rel="noopener"
                className="text-[#C9A84C]/70 hover:text-[#C9A84C] inline-flex items-center gap-0.5">
        ...
        <div>
          <label className="text-[10px] text-white/40 uppercase tracking-wider block mb-1.5">
            Label <span className="text-white/20">(ex: "pessoal", "produção")</span>
          </label>
          <input value={label} onChange={e => setLabel(e.target.value)}
            placeholder="Minha key de produção"
            className="w-full bg-black/30 border border-white/10 rounded-xl px-3 py-2.5 text-sm focus:outline-none focus:border-[#C9A84C]/40 transition-colors"
          />
        </div>
        ...
        <div>
          <label className="text-[10px] text-white/40 uppercase tracking-wider block mb-1.5">
            API Key <span className="text-white/20">— criptografada antes de salvar</span>
          </label>
          <div className="relative">
            <input type={show ? "text" : "password"} value={key}
              onChange={e => setKey(e.target.value)}
              placeholder={cfg?.placeholder ?? "sk-..."}
              className="w-full bg-black/30 border border-white/10 rounded-xl px-3 py-2.5 pr-10 text-sm font-mono focus:outline-none focus:border-[#C9A84C]/40 transition-colors"
            />
          ...
          <div className="flex items-center gap-1.5 mt-1.5">
            ...
            <p className="text-[10px] text-white/30">
              Criptografada com AES-256-GCM. O valor original nunca é armazenado.
            </p>
          </div>
        </div>
        ...
        <div className="flex gap-2 pt-1">
          ...
          <button type="button" onClick={() => setOpen(false)}
            className="px-4 py-2.5 rounded-xl text-sm border border-white/10 hover:border-white/25 transition-colors">
            Cancelar
          </button>
        </div>
      </form>
...
export default function KeysPage() {
  ...
  function addKey(k: ApiKey) { setKeys(prev => [k, ...prev]); }
  function removeKey(id: string) { setKeys(prev => prev.filter(k => k.id !== id)); }

  // Group by provider
  const byProvider: Record<string, ApiKey[]> = {};
  for (const k of keys) {
    ...
    <div className="p-8 max-w-3xl">
      ...
      <div className="mb-8">
        ...
        <p className="text-white/40 text-sm">
          Traga suas próprias API keys. Você paga direto nas APIs — sem markup.
          Criptografadas com AES-256-GCM antes de salvar.
        </p>
      </div>
      ...
      <div className="grid grid-cols-3 gap-3 mb-8">
        {[
          { label:"Configuradas", value:configuredProviders.size, of:Object.keys(PROVIDERS).length, color:"#C9A84C" },
      ...
      <div className="flex items-start gap-3 p-4 mb-6 bg-green-500/5 border border-green-500/20 rounded-2xl">
        ...
        <div>
          ...
          <p className="text-xs text-white/40 mt-0.5">
            Keys criptografadas com chave mestra separada (VAULT_KEY env var). O texto original nunca toca o banco.
            Só o hash SHA-256 é armazenado para verificação. Não é possível reverter.
          </p>
        </div>
      </div>
      ...
      {loading ? (
        ...
        <div className="text-center py-10 text-white/20 mb-6">
          ...
          <p className="text-sm">Nenhuma key configurada ainda</p>
          <p className="text-xs mt-1">Adicione suas keys abaixo para começar a gerar</p>
        </div>
      ) : (
        <div className="space-y-2 mb-6">
          {keys.map(k => <KeyCard key={k.id} apiKey={k} onDelete={removeKey}/>)}
        </div>
      )}

      {/* Add form */}
      <AddKeyForm onSuccess={addKey}/>

      {/* Provider guide */}
      <div className="mt-8 space-y-2">
        <p className="text-xs text-white/30 uppercase tracking-wider font-semibold">Providers disponíveis</p>
        {Object.entries(PROVIDERS).map(([id, p]) => (
          <div key={id} className="flex items-center gap-3 p-3 rounded-xl border border-white/5">
            ...
            {configuredProviders.has(id) ? (
              ...
              <a href={p.docsUrl} target="_blank" rel="noopener"
                className="text-[10px] text-white/25 hover:text-[#C9A84C] flex items-center gap-1 flex-shrink-0 transition-colors">
                Obter key <ExternalLink size={9}/>
              </a>
            )}
          </div>
